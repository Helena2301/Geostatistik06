...
bbox(SPDFljz)
#library(sf)
#new_bb <- c(490261, 3121075, 493771, 3125845)
#names(new_bb) <- c("xmin", "ymin", "xmax", "ymax")
#attr(new_bb, "class") <- "bbox"

#attr(SPDFljz, "bbox") <- new_bb
#bbox(SPDFljz)
...
extent(Rljz) <- c(490261, 493771, 3121075, 3125845)
dim(Rljz) <- c(14, 14) #vllt andere Pixelsize?
...
#GT <- GridTopology(cellcentre.offset = cc, #c(490261, 3121075)
#                   cellsize = cs,          #c(14, 14)
#                   cells.dim = cd)         #c(1920, 1080)
#SPG <- SpatialGrid(data= SPDljz, grid = GT, proj4string = coordRefSys)

vRljz <- rasterize(SPDFljz, Rljz, Ca)
spplot(vRljz)            
```
...
#Ordinary Kriging
library(krige)
target_raster <- as(vRljz, "SpatialGridDataFrame")
interpol_Ca <- gstat::krige(Ca ~ 1, 
                            SPDFljz, target_raster, 
                            model = vario_omni_Ca_fit)
#summary(interpol_Ca)
...
```{r Aufgabe 13d}
#is.na(interpol_Ca) <- interpol_Ca@data$var1.pred > 79.69
```
...
mycolor <- brewer.pal(n = 9, name = "Greys")
library(sp)
library(lattice)
mylayout <- list(col.pixels = mycolor)
spplot(interpol_Ca, "var1.pred",
